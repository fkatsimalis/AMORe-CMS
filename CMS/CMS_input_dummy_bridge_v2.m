...
% ONLY if local reduction with parametrization is used "indata.reduction_I=6"
indata.use_parallel=0; % =1 -> assembles the matrices in each sample point using a parfor loop (generally faster)
indata.use_interp=1; % =0 -> no interpolation is used. Instead, the interface modes are kept constant at the nominal point (much faster)
% ---
...
r=3*ones(1,100); % multiplication constant used to define the target frequency 2.5, 2.6, 2, 3
indata.eigf.group.multiplier=r;
indata.eigf.group.target=r*2000; % target eigenfrequency (Hz) for each group of components 2000, 4.6, 1300
...
indata.eigf.group.target_store=20*2000*ones(1,100); % large cutoff frequency
...
% this is used if method=1
v=20*ones(1,100); % multiplication constant used to define the target frequency
...
indata.eigf.interface.target=v*2000; % target eigenfrequency (Hz) for each interface
...
% this is used if method_store=1
indata.eigf.interface.target_store=20*2000*ones(1,100); % large cutoff frequency
...
indata.E=37*10^9*ones(22,1); % Young's modulus [Pa] for each group of components
indata.nu=.2*ones(22,1); % Poisson's ratio
indata.rho=2548*ones(22,1); % density [kg/m^3]
% ---
...
indata.func_g=repmat({@(x) 1},1,4);

% func_h applies on stiffnes matrix (see Eq. (2.4))
indata.func_h=repmat({@(x) x},1,4);
